...
\begin{acronym}
    \acro{sspx1}[SSP-X1]{Smart Sensor Platform X1}
    \acro{opcua}[OPC UA]{Open Platform Communication Unified Architecture}
    \acro{gpu}[GPU]{Graphics Processing Unit}
    \acro{cpu}[CPU]{Central Processing Unit}
    \acro{iot}[IoT]{Internet of Things}
    \acro{sql}[SQL]{Structured Query Language}
    \acro{prkurve}[PR-Kurve]{Precision-Recall-Kurve}
    \acroplural{prkurve}[PR-Kurven]{Precision-Recall-Kurven}
    \acro{aucpr}[AUC-PR]{Area Under the Precision-Recall-Curve}
    \acro{ekg}[EKG]{Elektrokardiogramm}
    \acro{hbos}[HBOS]{Histogram-Based Outlier Score}
    \acro{swifd}[SWIFD]{Sliding Window Isolation Forest Density}
    \acro{md-swifd}[MD-SWIFD]{Mahalanobis Distanz mit Sliding Window Isolation Forest Density}
    \acro{lstm}[LSTM]{Long Short Term Memory}
    \acro{lstm-ae}[LSTM-AE]{Long Short Term Memory Autoencoder}
    \acro{swz}[SWZ]{Sliding Window Z-Score}
    \acro{ee}[EE]{Elliptic Envelope}
    \acro{ram}[RAM]{Rapid Access Memory}
